...
if( isset($_SESSION["user_name"]) && isset($_SESSION["password"]) && isset($_SESSION["user_type"]))
{
	...
	}else if($_SESSION["user_type"]=="BSC")
	{
		header('location: pages/empbsc/empbsc.php');
	}
	else if($_SESSION["user_type"]=="CBE")
	{
		header('location: pages/empcbe/empcbe.php');
	}
	else if($_SESSION["user_type"]=="MG")
	{
		header('location: pages/manager/manager.php');
	}
	else if($_SESSION["user_type"]=="APP")
	{
...
{
	header('location: controller/account/loginsession.php');
}else
{
...
<head>
	...
	<script language="javascript">
		...
		function formValidation()
		{
			...
			ALERET(username);
			if(username_validator(username) || applcantusername_validator(username))
			{
			...
			<div class="col-md-12 col-xs-12">
				<div class="row no-gutters">
					<div class="col-md-12 col-xs-12 hidden-md hidden-lg">
								<center><h4>Login</h4></center>
										<form action="controller/account/login.php" method="Post" name="login" onsubmit="return formValidation()">
											<div class="form-group" id="usernamediv">
			                                    <label class="control-label" for="username">Username</label>
			                                    <input type="text" class="form-control" id="username" name="username" onchange="validation_state()">
			                                    <div class="valid-feedback" id="username_validation_feedback"></div>
											...
											<input type="button" class="btn btn-danger" style="float:right;" id="login" value="Login" onclick='w()'>
										</form>
